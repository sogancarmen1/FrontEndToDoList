...
<template>
    <div v-if="reveleUpdateTask" class="flex items-center justify-center h-screen fixed top-0 bottom-0 right-0 left-0">
        <div @click="tooglereveleUpdateTask" class="bg-[rgba(0,0,0,0.5)] fixed top-0 bottom-0 right-0 left-0"></div>
        <div class="space-y-2 border rounded-lg bg-white shadow-2xl fixed">
            ...
            <!-- <div class="flex"> -->
                <div class="px-8 pt-4 flex items-center flex-none">
                    <div>
                        <div>
                            <img class="py-2 ml-16 w-28 rounded-bl-lg rounded-tl-lg" src="../assets/trello.png" alt="">
                            <p class="after:content-['*'] after:ml-0.5 after:text-red-500 block text-sm font-medium text-slate-700">Name</p>
                            <input :value="nameUpdate" @input="nameUpdate = $event.target.value" placeholder="Enter name of task" type="" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1"/>
                        </div>
                        <div>
                            <p class="after:content-['*'] after:ml-0.5 after:text-red-500 block text-sm font-medium text-slate-700">Due Date</p>
                            <input :value="dueDateUpdate" @input="dueDateUpdate = $event.target.value" placeholder="Enter name of task" type="date" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1"/>
                        </div>
                        <div>
                            <p class="after:content-['*'] after:ml-0.5 after:text-red-500 block text-sm font-medium text-slate-700">Priority</p>
                            <select :value="priorityUpdate" @input="priorityUpdate = $event.target.value" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1">
                        ...
                        <div>
                            <p class="after:content-['*'] after:ml-0.5 after:text-red-500 block text-sm font-medium text-slate-700">Status</p>
                            <select :value="statusUpdate" @input="statusUpdate = $event.target.value" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1">
